...
export const userSettingsFormSchema = Yup.object().shape({
  ...
  email: Yup.string()
    .email(
      i18n.t("validationSettings.email.email", {
        defaultValue: "Please enter a valid email address.",
      })
    )
    .typeError(
      i18n.t("validationSettings.email.typeError", {
        defaultValue: "Email must be a string",
      })
    )
    .required(
      i18n.t("validationSettings.email.required", {
  ...
  sportTime: Yup.number()
    .typeError(
      i18n.t("validationSettings.activityTime.typeError", {
        defaultValue: "Sport time must be a number",
      })
    )
    .min(
      0,
      i18n.t("validationSettings.activityTime.min", {
        defaultValue: "Sport time cannot be negative.",
      })
    )
    .max(
      24,
      i18n.t("validationSettings.activityTime.max", {
  ...
  weight: Yup.number()
    .typeError(
      i18n.t("validationSettings.weight.typeError", {
        defaultValue: "Weight must be a number",
      })
    )
    .min(
      0,
      i18n.t("validationSettings.weight.min", {
        defaultValue: "Weight cannot be negative.",
      })
    )
    .max(
      350,
      i18n.t("validationSettings.weight.max", {
  ...
  waterNorm: Yup.number()
    .typeError(
      i18n.t("validationSettings.desiredVolume.typeError", {
        defaultValue: "Water intake must be a number",
      })
    )
    .min(
      0.05,
      i18n.t("validationSettings.desiredVolume.min", {
        defaultValue: "Water intake must be at least 0.05 L.",
      })
    )
    .max(
      5,
      i18n.t("validationSettings.desiredVolume.max", {
        defaultValue: "Water intake cannot exceed 5 L.",
      })
    )
    .required(
      i18n.t("validationSettings.desiredVolume.required", {
